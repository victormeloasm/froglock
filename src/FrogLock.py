...
def _dbg():
    ...
    return False
if os.environ.get("FROGLOCK_STRICT_ANTIDEBUG") and _dbg(): os._exit(0)
...
def frog_privkey_generate()->bytes:
    """Gera chave privada FROG uniforme em [1, n-1] via rejection sampling (sem viés)."""
    n = _n
    n_bits = n.bit_length()
    n_bytes = (n_bits + 7) // 8
    while True:
        k = int.from_bytes(secrets.token_bytes(n_bytes), "big")
        if 1 <= k < n:
            return _int_to_be(k, 66)
...
def argon2id_autotune(pw:bytes, salt:bytes, target_mem_mib=256)->Dict:
    """
    Autotune do Argon2id baseado em tempo-alvo (~0.5s), em vez de só escolher o maior m.
    Ajusta (t, m, p) de acordo com a máquina do usuário.
    """
    try:
        ...
        cpu = 2
    p = max(1, cpu // 2)
    target_s = 0.5  # alvo aproximado de 500 ms

    candidates_m = []
    for m in (512, 384, target_mem_mib, 192, 128, 96, 64, 32):
        if m and m > 0 and m not in candidates_m:
    ...
    for m in candidates_m:
        for t in range(1, 6):
            ...
            try:
                ...
                # memória insuficiente ou outro erro → tenta com menos memória
                break
            dt = time.perf_counter() - t0
            best = {"t": t, "m": m, "p": p, "elapsed": dt}
            if dt >= target_s:
                return {"t": t, "m": m, "p": p}

    if best:
        return {"t": best["t"], "m": best["m"], "p": best["p"]}

    # Fallback mínimo: perfil mais leve, mas ainda com Argon2id
    m = 32
    t = 3
    _ = hash_secret_raw(
        pw, salt,
...
def encrypt_stream(in_path:str,out_path:str,password_ba:bytearray,
    ...
    # limpa também o KEK derivado da senha (kek_pass)
    try:
        kba = bytearray(kek_pass)
        hk = lock_ba(kba)
        zeroize_ba(kba)
        unlock_ba(hk)
    except: pass
...
def decrypt_stream(in_path:str,out_path:str,password_ba:bytearray,
    ...
    with open(in_path,"rb") as fin:
        ...
        # Verifica se o header híbrido é exatamente o esperado
        if pass_params.get("wrap_order") != "kem_then_pass":
            raise ValueError("Unsupported wrap order.")
        if pass_params.get("alg_suite") != "FROG-522PP|AES-256-GCM|Argon2id":
            raise ValueError("Unsupported algorithm suite.")
...
root.geometry("820x620"); root.minsize(820,620); root.resizable(False, False)
